      ...
      <div className={`w-full ${contactSection[1] ? 'animate-riseUpSection' : 'opacity-0'}`}>
        <div className="card p-6 sm:p-8 soft-shadow" data-aos="fade-up">
          ...
          <form action="/api/contact" method="post" className="mt-6 grid gap-4">
            ...
            <textarea name="message" required placeholder="Tell me about your project" className={`rounded-xl border border-gray-200 bg-white/80 px-5 py-3 min-h-28 text-base focus:outline-none focus:ring-2 focus:ring-[color:var(--accent)] transition-all duration-200 shadow-sm placeholder:text-gray-400 resize-none ${chewy.className}`} style={{ fontWeight: 400, letterSpacing: '0.04em' }} />
            <button className={`relative group rounded-xl px-6 py-3 font-semibold soft-shadow bg-[color:var(--accent)] text-white hover:bg-[color:var(--accent)]/90 transition-all duration-200 w-fit mx-auto shadow-md border-0 ${chewy.className}`} style={{ fontWeight: 400, letterSpacing: '0.04em', boxShadow: '0 2px 12px rgba(44,54,90,0.10)' }}>
              <span className="hidden sm:block absolute left-1/2 -translate-x-1/2 -top-8 text-3xl transition-all duration-300 opacity-0 group-hover:opacity-100 group-hover:-translate-y-2 pointer-events-none select-none" aria-hidden="true" style={{ zIndex: 10 }}>🖐️</span>
              Send
            </button>
          </form>
